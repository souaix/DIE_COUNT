...
def die_count():
	...
	try:
		...
		if '0 received' not in msg:
			...
			if (mo_body in ftp.nlst()) == True:
				downloadfile(ftp, "/WAFERTRAN6B/"+mo_body, dl_path)
				df_6B = pd.read_csv(dl_path)
				df_6B = df_6B[["Data create","Op id","Mo #","Frame barcode", "Qty"]]
				df_6B.replace('\s', '', regex=True, inplace=True)
			else:
				ftp.cwd(yy)
				if (mo_body in ftp.nlst()) == True:
					downloadfile(ftp, "/WAFERTRAN6B/"+yy+"/"+mo_body, dl_path)
					df_6B = pd.read_csv(dl_path)
					df_6B = df_6B[["Data create","Op id","Mo #","Frame barcode", "Qty"]]
					df_6B.replace('\s', '', regex=True, inplace=True)
				else:
	...
	try:
		...
		if '0 received' not in msg:
			...
			if (mo_body in ftp.nlst()) == True:
				downloadfile(ftp, "/WAFERTRAN8A/"+mo_body, dl_path)
				df_8A = pd.read_csv(dl_path)
				df_8A = df_8A[["Data create","Op id","Mo #","Frame barcode", "Qty"]]
				df_8A.replace('\s', '', regex=True, inplace=True)
			else:
				ftp.cwd(yy)
				if (mo_body in ftp.nlst()) == True:
					downloadfile(ftp, "/WAFERTRAN8A/"+yy+"/"+mo_body, dl_path)
					df_8A = pd.read_csv(dl_path)
					df_8A = df_8A[["Data create","Op id","Mo #","Frame barcode", "Qty"]]
					df_8A.replace('\s', '', regex=True, inplace=True)
				else:
	...
	try:
		...
		if '0 received' not in msg:
			...
			if (mo_body in ftp.nlst()) == True:
				downloadfile(ftp, "/WAFERTRAN8B/"+mo_body, dl_path)
				df_8B = pd.read_csv(dl_path)
				df_8B = df_8B[["Data create","Op id","Mo #","Frame barcode", "Qty"]]
				df_8B.replace('\s', '', regex=True, inplace=True)
			else:
				ftp.cwd(yy)
				if (mo_body in ftp.nlst()) == True:
					downloadfile(ftp, "/WAFERTRAN8B/"+yy+"/"+mo_body, dl_path)
					df_8B = pd.read_csv(dl_path)
					df_8B = df_8B[["Data create","Op id","Mo #","Frame barcode", "Qty"]]
					df_8B.replace('\s', '', regex=True, inplace=True)
				else:
